\setsection{Introduction}
	...
	\setsubsection{Historique}
		...
		21 ans après, Oechslin\cite{Oech03} propose une autre amélioration du compromis temps-mémoire, différente de celle de Rivest, les \gls{rainbows}. Deux ans plus tard, Gildas Avoine, Pascal Junod et Philippe Oechslin\cite{checkpoints} proposent une avancée pour le compromis, l'ajout de points de contrôles (points utilisables aussi avec les points distincts ou les \gls{rainbows}).

		\bigskip

		Enfin, en 2006, Elad Barkan propose une variante se basant sur les \gls{rainbows} et la méthode des points distincts\cite{fuzzy}, les \gls{rainbows} floues. Puis en 2008, Jin Hong, Kyung Jeong, Eun Kwon, In-Sok Lee, et Daegun Ma proposent une variantes à la méthode des points distincts. Depuis ces variantes, aucune avancée majeure n'a été publiée sur ce sujet.
	...
	\setsubsection{Origines}
		...
		Afin de comprendre les \gls{rainbows} et leurs avantages par rapport à d'autres méthodes, nous allons commencer par expliquer le principe du compromis temps-mémoire. Puis nous présenterons les \gls{rainbows} en les comparant à la méthode des points distincts. Nous détaillerons ensuite cette méthode des points distincts. Enfin nous verrons une amélioration notable de ces tables.
